...
func get_nodes_by_partial_node_name(node_name: String):
	if navmesh :
		var nav_mesh_children = navmesh.get_children()
		var node_list = Array()
		
		for node in nav_mesh_children:
			if node.name.contains(node_name):
...
func set_object_data():
	var object_nodes = get_nodes_by_partial_node_name('Object')
...
func set_entity_data():
	var entity_nodes = get_nodes_by_partial_node_name('Entity')
